...
seahub_version=${PV}-server
# This version uses an earlier seahub
seahub_version=4.3.2-server
...
then
	...
	SRC_URI="https://github.com/haiwen/seafile/archive/v${PV}.tar.gz -> ${P}.tar.gz"
	KEYWORDS="~x86 ~amd64"
	S="${WORKDIR}/seafile-${PV}"
fi
...
DEPEND="dev-lang/python
	...
	client? (
	   =net-libs/ccnet-${PV}[client]
	)
    server? (
	   =net-libs/ccnet-${PV}[server]
	)
	python? (
...
src_install() {
	...
	if use server ; then
		...
		if use seahub ; then
			wget https://github.com/haiwen/seahub/archive/v${seahub_version}.tar.gz --output-document seahub.tar.gz
			tar xzf seahub.tar.gz
			mv seahub-${seahub_version} seahub
			rm seahub.tar.gz
		fi
